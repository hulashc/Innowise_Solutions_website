    ...
    <footer className="border-t border-black/10 py-16 px-6">
      <div className="max-w-[1200px] mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10 md:gap-12">
          <div className="flex flex-col gap-6">
            ...
            <div className="flex gap-4">
              <span className="text-black/45" aria-label="Facebook">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                </svg>
              </span>
              <span className="text-black/45" aria-label="Twitter / X">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                </svg>
              </span>
              <span className="text-black/45" aria-label="Instagram">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <Link href="/services" className="text-black/65 hover:text-black text-sm transition-all duration-200">Services</Link>
              <Link href="/blog" className="text-black/65 hover:text-black text-sm transition-all duration-200">Blog</Link>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2 text-sm text-black/65">
              ...
              <a href="tel:+441162257865" className="hover:text-black transition-all duration-200">+44 0116 225 7865</a>
            </div>
            <div className="mt-2">
              {subscribed ? (
                <p className="text-black/55 text-xs">Thanks for subscribing!</p>
              ) : (
                <form onSubmit={(e) => { e.preventDefault(); setSubscribed(true); }} className="flex">
                  <input
                    type="email"
                    placeholder="Your email"
                     className="bg-black/5 border border-black/10 rounded-full px-4 py-2 text-sm text-black placeholder-black/50 flex-1 outline-none focus:border-black/30 transition-all duration-200"
                  />
                  <button
                    type="submit"
                     className="bg-black/5 border border-black/10 rounded-full px-4 py-2 text-sm text-black/65 hover:bg-black/10 hover:text-black -ml-2 transition-all duration-200 cursor-pointer"
                  >
                    Subscribe
                  </button>
                </form>
        ...
        <div className="mt-16 pt-6 border-t border-black/5 text-center">
          <p className="text-black/35 text-xs">&copy; 2012 Innowise Solutions Ltd. All rights reserved.</p>
        </div>
      </div>
    </footer>
  );
