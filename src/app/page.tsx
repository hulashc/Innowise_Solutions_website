    ...
    <>
      ...
      <section className="h-dvh p-2 md:p-2 overflow-hidden">
        <div className="relative w-full h-full rounded-xl md:rounded-2xl border border-white overflow-hidden">
          ...
          {/* Content fills the rectangle */}
          <div className="relative z-10 w-full h-full px-4 md:pr-10 lg:pr-14 md:pl-4 lg:pl-6 flex flex-col md:pt-24 lg:pt-32">
            {/* Spacer for mobile to push heading toward middle */}
    ...
    >
      <div className="aspect-[4/3] relative overflow-hidden">
        <motion.div className="absolute inset-0" style={{ y }}>
          <Image src={service.img} alt="" fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover scale-110" />
        </motion.div>
        <motion.div className="absolute inset-0 bg-black/40 group-hover:bg-black/20 transition-colors duration-300" />
        <div className="absolute bottom-0 left-0 right-0 p-4 md:p-6 bg-gradient-to-t from-black/80 via-black/30 to-transparent">
          <motion.p className="text-white/90 text-xs uppercase tracking-widest font-medium">
            {service.title}
          </motion.p>
        </div>
      </div>
...
const messages = [
  { side: "left", avatar: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=48&q=80", text: "Hey, we need to scale our infrastructure ASAP." },
  { side: "right", avatar: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=48&q=80", text: "On it. I'll deploy the auto-scaling cluster now." },
  { side: "left", avatar: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=48&q=80", text: "Great. And set up monitoring for the new endpoints?" },
  { side: "right", avatar: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=48&q=80", text: "Already configured. Alerts will hit Slack in real-time." },
  { side: "left", avatar: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=48&q=80", text: "Perfect. We're live in 10 minutes." },
  { side: "right", avatar: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=48&q=80", text: "\uD83D\uDE80 Deploying now." },
];
        ...
        <AnimatePresence mode="popLayout">
          {messages.slice(0, step).map((msg, i) => (
            <motion.div
              ...
              layout
              className={`flex items-end gap-2 max-w-[80%] mb-2 ${msg.side === "right" ? "self-end" : ""}`}
            >
              {msg.side === "left" && (
                <div className="relative w-6 h-6 rounded-full overflow-hidden shrink-0 mb-0.5">
              ...
              {msg.side === "right" && (
                <div className="relative w-6 h-6 rounded-full overflow-hidden shrink-0 mb-0.5">
                  <Image src={msg.avatar} alt="Support agent avatar" fill className="object-cover" />
                </div>
              )}
            </motion.div>
          ))}
